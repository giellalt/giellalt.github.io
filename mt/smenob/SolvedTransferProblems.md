...
Bokmålsordboka bøyer _mat_ i fleirtal, og det finst
(heilt marginale) døme på nett. Sjølv vil eg gjerne ha det
berre i eintal i nob-morfologi, men problemet vi har må
løysast i transfer: Paradigme for _mat_ utan +Pl
gjev oss #, og paradigme med sg = pl for _mat_ vil
gje oss former med adjektiv i fleirtal
(_14 gode mat til en bok_). Ei løysing er:
...
Her må vi lage nytt paradigme for _rein_ (det er kanskje gjort), der vi bøyer _rein_ som _ting_ (flere rein, ting)
...
Også her vil vi ha nytt paradigme for _million_
...
**Generaliseringa er:** \\
Samisk _Lea N ahte S_ skal vere _Det er en/ei/et N at S_
...
Trond: Poenget mitt var at _Lea N ahte S_ gjeld **alle** N, ikkje berre
ei undergruppe av substantiva. Eg vil tru at _maydetind_ er
...
Norsk skal ha ubestemt form etter negasjon av typen _ikke noe_ (litt som partitiv i finsk)

Lene:

- For denne setninga: Med enkel nekting får vi ubestemt for vi da får ext-tagg. Jeg forbedrer dette i functions.
- legger til veaháš som pron til adj i bidix, og da får vi ubestemt form, fordi ordet blir med i chunken: - 2001 prekte Norges prinsesse Märtha Louise på samisk, selv om hun hun har ikke ikke #noe samiskbakgrunn.
  /// LØYST ///
...
Med berre _vástideaba_ får vi _på norsk_, men med heile setninga
får vi korrekt _til norsk_.
...
"eaktun" _som betingelse_ -> _en betingelse_.
...
- _eaktun_ bør på liste over essivar som skal omsetjast med _en_ og ikkje med _som_
...
_Redaktørens bøkene_ er alltid feil
...
Problemet er at _samisk_ blir analysert som substantiv.
...
(det autentiske dømet) _Norsk arktisk torsks dumpingssalg gjennom årtider._
Derimot finn eg ikkje _engelsks_ på Google. Eg skal sjå om det er
...
Eit akronym som både har dette leksikonet og står i bidix er _AS_, jf:
...
- For fleirtals-N sett vi ikkje inn _en_
- _en_ skal ikkje inn for ord for:
...
Vi treng ei liste over slike (og undersøke unntaka der _i arbeidet_ er ok).

FIXED for sem_act når hovedverbet er til venstre og er _leat_.
...
Ein setningsinitial streng _AN_ skal bli _en A N_.
...
Alle N2-gen N1 som blir overført til _N1 til N2_ skal ha bestemt N1 og ubestemt N2.
...
Konstruksjonen `passivsubjekt + lea + V-juvvon` skal ha _være_ som hjelpeverb, ikkje _ha_
...
_jeg_ kjem pga Sg1 på -huvvon, som kjem fordi det ikkje er
...
I dag har vi det-insetjing (fint), men _om_ manglar, og ordstillinga er feil.
...
Det som skjer er at adverbet _gitta_ blir analysert som del av NPen, og dermed som del av den resulterande PPen.
_gitta_ må på ei liste over adverb som ikkje skal vere del av NPen, det gjeld kanskje til og med alle adverb.
