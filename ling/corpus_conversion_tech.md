...
- **corpus_chmod.sh:**  
  ...
  sudo rights.
- **change_xsl_generic.py:**  
  for i in \`find . -name \\\*.xsl\`  
  do  
  change_xsl_generic.py translator_fn Pirjo translator_ln
  Paavaniemi  
  translator_gender fem translator_nat FI $i  
  done
- **gt2ga.sh:**  
...
. Some of the code tables like samimac_roman and levi_winsam are
...
"combine_two_codings($coding1, $coding2, $outfile)" which is available
in this package.
...
listed in hash %Char_Tables for North Saami for example. The
