...
To search for more advanced expressions you use single or double quotation marks. The command `grep mii filename` in a Sami text finds both _mii_ and _huksemiid_. To search only for _mii_ at the beginning of words you may write `grep ' mii' filename`. To search for the word _mii_ you may write `grep -w mii filename`
...
Summary:
The pipe symbol **|** _pipes_ the output of one command as input to the next.
