...
-   Páhkat resources:
    ...
    -   Keyboards: edit existing `locales:` and add more to the file `XXX.kbdgen/project.yaml` (replace `XXX` with the language ISO 639-3 code):
        ...
        - The new/edited locales are pushed to the Páhkat server on the next successfull CI build, and from there it is automatically picked up by borealium.org on the next site build (see above) [**NB!** This seems not to work ATM.]
